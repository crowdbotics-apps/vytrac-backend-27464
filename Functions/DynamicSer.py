...
class DynamicSerializer(QueryFieldsMixin, serializers.ModelSerializer):
    def __init__(self, *args, **kwargs):
        ...
        # Debugging(kwargs['context']['request'].__dict__, color='red')
        # Debugging(kwargs['context']['view'].__class__.__name__)
        pk = None
        user = None
        method = ''
        is_owner = False
        ...
        #         if not permission['fields'] is None:
        #             view_fields = permission['fields'] if method == 'view' else None
        #             change_fields = permission['fields'] if method == 'change' else None
        #     else:
